 ...
 * the committer.
 */
public class S3AOpenAIRECommitterFactory extends AbstractS3ACommitterFactory {

	/**
	 * Name of this class: {@value}.
	 */
	public static final String CLASSNAME
			= "org.apache.hadoop.fs.s3a.commit.S3AOpenAIRECommitterFactory";
	...
	public PathOutputCommitter createTaskCommitter(S3AFileSystem fileSystem,
		...
		if (factory != null) {
			...
			AbstractS3ACommitterFactory.LOG.info("Using committer {} to output data to {}",
					(committer instanceof AbstractS3AOpenAIRECommitter
							? ((AbstractS3AOpenAIRECommitter) committer).getName()
							: committer.toString()),
					outputPath);
			return committer;
		} else {
			AbstractS3ACommitterFactory.LOG.warn("Using standard FileOutputCommitter to commit work."
	...
	private AbstractS3ACommitterFactory chooseCommitterFactory(
		...
		String name = fsConf.getTrimmed(CommitConstants.FS_S3A_COMMITTER_NAME, "");
		AbstractS3ACommitterFactory.LOG.debug("Committer from filesystems \"{}\"", name);

		name = taskConf.getTrimmed(CommitConstants.FS_S3A_COMMITTER_NAME, name);
		AbstractS3ACommitterFactory.LOG.debug("Committer option is \"{}\"", name);
		switch (name) {
			case CommitConstants.COMMITTER_NAME_FILE:
			case "":
				factory = null;
				break;
			case CommitConstants.COMMITTER_NAME_DIRECTORY:
				factory = new DirectoryStagingCommitterFactory();
				break;
			case CommitConstants.COMMITTER_NAME_PARTITIONED:
				factory = new PartitionedStagingCommitterFactory();
				break;
			case CommitConstants.COMMITTER_NAME_MAGIC:
				factory = new MagicS3OpenAIRECommitterFactory();
				break;
