...
public class MagicS3OpenAIRECommitter extends AbstractS3AOpenAIRECommitter {
	private static final Logger LOG =
			LoggerFactory.getLogger(MagicS3OpenAIRECommitter.class);
	...
	public static Path tempDirForStaging(FileSystem fs,
		...
		Path path =  new Path(conf.getTrimmed(
	 ...
	 */
	public MagicS3OpenAIRECommitter(Path outputPath,
									TaskAttemptContext context) throws IOException {
		super(outputPath, context);
		stagingDir = tempDirForStaging(FileSystem.get(getConf()), getConf());
	...
	protected ActiveCommit listPendingUploadsToCommit(
			...
			throws IOException {
		FileSystem fs = getDestinationFS(stagingDir, getConf());
	...
	public void cleanupStagingDirs() {
		Path path = getMagicJobPath(getUUID(), stagingDir);
	...
	private PendingSet innerCommitTask(
		...
		try (CommitContext commitContext = initiateTaskOperation(context)) {
			...
			try {
				...
				pendingSet.save(taskOutcomePath.getFileSystem(getConf()),
						taskOutcomePath,
	...
	protected Path getJobPath() {
		return getMagicJobPath(getUUID(), stagingDir);
	}
	...
	protected final Path getJobAttemptPath(int appAttemptId) {
		return getMagicJobAttemptPath(getUUID(), appAttemptId, stagingDir);
	}
