...
cat step20-createMonitorDB_funded.sql | sed "s/SOURCE/$SOURCE/g" | sed "s/TARGET/${TARGET}_funded/g1" > foo
hive $HIVE_OPTS -f foo
cat step20-createMonitorDB.sql | sed "s/SOURCE/$SOURCE/g" | sed "s/TARGET/${TARGET}_funded/g1" > foo
hive $HIVE_OPTS -f foo

cat step20-createMonitorDB_institutions.sql | sed "s/SOURCE/$SOURCE/g" | sed "s/TARGET/${TARGET}_institutions/g1" > foo
hive $HIVE_OPTS -f foo
cat step20-createMonitorDB.sql | sed "s/SOURCE/$SOURCE/g" | sed "s/TARGET/${TARGET}_institutions/g1" > foo
hive $HIVE_OPTS -f foo
...
cat step20-createMonitorDB_RIs_tail.sql | sed "s/SOURCE/$SOURCE/g" | sed "s/TARGET/${TARGET}_ris_tail/g1" | sed "s/CONTEXTS/\"'knowmad::other','dh-ch::other', 'enermaps::other', 'gotriple::other', 'neanias-atmospheric::other', 'rural-digital-europe::other', 'covid-19::other', 'aurora::other', 'neanias-space::other', 'north-america-studies::other', 'north-american-studies::other', 'eutopia::other'\"/g" > foo
hive $HIVE_OPTS -f foo
cat step20-createMonitorDB.sql | sed "s/SOURCE/$SOURCE/g" | sed "s/TARGET/${TARGET}_ris_tail/g1" > foo
hive $HIVE_OPTS -f foo
...
hive $HIVE_OPTS --database ${TARGET}_funded -e "show tables" | grep -v WARN | sed "s/\(.*\)/create view ${SHADOW}_funded.\1 as select * from ${TARGET}_funded.\1;/" > foo
hive -f foo
...
hive $HIVE_OPTS --database ${TARGET}_institutions -e "show tables" | grep -v WARN | sed "s/\(.*\)/create view ${SHADOW}_institutions.\1 as select * from ${TARGET}_institutions.\1;/" > foo
hive -f foo
...
do
  ...
  hive $HIVE_OPTS --database ${TARGET}_${tmp} -e "show tables" | grep -v WARN  | sed "s/\(.*\)/create view ${SHADOW}_${tmp}.\1 as select * from ${TARGET}_${tmp}.\1;/" > foo
  hive -f foo
done
...
hive $HIVE_OPTS --database ${TARGET}_ris_tail -e "show tables" | grep -v WARN | sed "s/\(.*\)/create view ${SHADOW}_ris_tail.\1 as select * from ${TARGET}_ris_tail.\1;/" > foo
hive -f foo
