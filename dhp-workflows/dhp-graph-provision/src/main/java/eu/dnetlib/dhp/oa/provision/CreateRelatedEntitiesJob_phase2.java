...
public class CreateRelatedEntitiesJob_phase2 {
  ...
  private static void joinAllEntities(
    ...
    Dataset<Tuple2<String, TypedRow>> entities =
        readAllEntities(spark, inputGraphRootPath, numPartitions);
    Dataset<Tuple2<String, EntityRelEntity>> relsBySource =
        readRelatedEntities(spark, inputRelatedEntitiesPath);

    entities
        .joinWith(relsBySource, entities.col("_1").equalTo(relsBySource.col("_1")), "left_outer")
        .map(
            (MapFunction<
                    Tuple2<Tuple2<String, TypedRow>, Tuple2<String, EntityRelEntity>>,
                    EntityRelEntity>)
                value -> {
                  EntityRelEntity re = new EntityRelEntity();
                  re.setEntity(value._1()._2());
                  Optional<EntityRelEntity> related =
        ...
        .filter(
            (FilterFunction<EntityRelEntity>)
                value ->
                    value.getEntity() != null && StringUtils.isNotBlank(value.getEntity().getId()))
        .write()
  ...
  private static Dataset<Tuple2<String, TypedRow>> readAllEntities(
    ...
    Dataset<TypedRow> publication =
        readPathEntity(spark, inputGraphPath + "/publication", Publication.class);
    Dataset<TypedRow> dataset =
        readPathEntity(spark, inputGraphPath + "/dataset", eu.dnetlib.dhp.schema.oaf.Dataset.class);
    Dataset<TypedRow> other =
        readPathEntity(spark, inputGraphPath + "/otherresearchproduct", OtherResearchProduct.class);
    Dataset<TypedRow> software =
        readPathEntity(spark, inputGraphPath + "/software", Software.class);
    Dataset<TypedRow> datasource =
        readPathEntity(spark, inputGraphPath + "/datasource", Datasource.class);
    Dataset<TypedRow> organization =
        readPathEntity(spark, inputGraphPath + "/organization", Organization.class);
    ...
    return publication
        ...
        .union(project)
        .map(
            (MapFunction<TypedRow, Tuple2<String, TypedRow>>)
  ...
  private static Dataset<Tuple2<String, EntityRelEntity>> readRelatedEntities(
    ...
    final List<String> paths =
        HdfsSupport.listFiles(inputRelatedEntitiesPath, spark.sparkContext().hadoopConfiguration());
    ...
    return spark
        ...
        .map(
            (MapFunction<EntityRelEntity, Tuple2<String, EntityRelEntity>>)
    ...
    return spark
        ...
        .map(
            (MapFunction<E, TypedRow>)
                value -> getTypedRow(StringUtils.substringAfterLast(inputEntityPath, "/"), value),
