...
public class SparkPropagateRelationsJob {
    ...
    public static void main(String[] args) throws Exception {
        final ArgumentApplicationParser parser =
                new ArgumentApplicationParser(
                        IOUtils.toString(
                                SparkPropagateRelationsJob.class.getResourceAsStream(
        ...
        final Dataset<Relation> rels =
                spark.read().load(relationPath).as(Encoders.bean(Relation.class));

        final Dataset<Relation> firstJoin =
                rels.joinWith(merge, merge.col("target").equalTo(rels.col("source")), "left_outer")
                        .map(
    ...
    private static String replaceField(final String json, final String id, final FieldType type) {
        ...
        try {
            ...
            if (relation.getDataInfo() == null) relation.setDataInfo(new DataInfo());
