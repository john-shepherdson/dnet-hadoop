...
public class DedupRecordFactory {
  ...
  public static JavaRDD<OafEntity> createDedupRecord(
    ...
    // <id, json_entity>
    final JavaPairRDD<String, String> inputJsonEntities =
        sc.textFile(entitiesInputPath)
            .mapToPair(
  ...
  private static Publication publicationMerger(Tuple2<String, Iterable<String>> e, final long ts) {
    ...
    if (p.getDataInfo() == null) p.setDataInfo(new DataInfo());
  ...
  private static Dataset datasetMerger(Tuple2<String, Iterable<String>> e, final long ts) {
    ...
    if (d.getDataInfo() == null) d.setDataInfo(new DataInfo());
  ...
  private static Software softwareMerger(Tuple2<String, Iterable<String>> e, final long ts) {
    ...
    if (s.getDataInfo() == null) s.setDataInfo(new DataInfo());
  ...
  private static Organization organizationMerger(
    ...
    if (o.getDataInfo() == null) o.setDataInfo(new DataInfo());
              ...
              orp -> {
                try {
                  OtherResearchProduct otherResearchProduct =
                      mapper.readValue(orp, OtherResearchProduct.class);
