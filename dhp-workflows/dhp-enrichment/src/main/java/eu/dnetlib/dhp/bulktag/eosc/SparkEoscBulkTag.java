...
public class SparkEoscBulkTag implements Serializable {
    ...
    public static void main(String[] args) throws Exception {
        ...
        runWithSparkSession(
                ...
                spark -> {
                    ...
                    execBulkTag(spark, inputPath, outputPath, datasourceMapPath, resultClazz);
                });
    }
    ...
    private static <R extends Result> void execBulkTag(
        ...
        final List<String> hostedByList = Arrays.asList(readPath(spark, datasourceMapPath, DatasourceMaster.class)
    ...
    private static <R extends Result> R enrich(R value, List<String> hostedByList) {
        if(value.getInstance().stream().anyMatch(i -> hostedByList.contains(i.getHostedby().getKey())) ||
            ...
            OafMapperUtils.dataInfo(false, BULKTAG_DATA_INFO_TYPE,true,false,
                    OafMapperUtils.qualifier(CLASS_ID_DATASOURCE, CLASS_NAME_BULKTAG_DATASOURCE,
                            DNET_PROVENANCE_ACTIONS, DNET_PROVENANCE_ACTIONS), TAGGING_TRUST);
