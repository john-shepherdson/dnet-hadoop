...
/**
 * This job is responsible of the creation of RAW Graph It is applied to the different entities
 * generated from {@link SparkExtractEntitiesJob} In case of dataset, publication and Unknown
 * Entities we group all the entities of the same type by their identifier, and then in the reduce
 * phase we merge all the entities. Merge means: -merge all the metadata -merge the collected From
 * values
 *
 * <p>In case of relation we need to make a different work: -Phase 1: Map reduce jobs Map: Get all
 * Relation and emit a key constructed by (source, relType, Target) and the relation itself Reduce:
 * Merge all relations Looking at the javadoc of {@link SparkSXGeneratePidSimlarity} we take the
 * dataset of pid relation and joining by source and target we replace the wrong identifier in the
 * relation with the correct ones. At the end we replace the new Dataset of Relation
 */
public class SparkScholexplorerCreateRawGraphJob {
  ...
  public static void main(String[] args) throws Exception {
    ...
    List<Path> subFolder =
        Arrays.stream(fs.listStatus(new Path(inputPath)))
