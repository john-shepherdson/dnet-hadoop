...
CREATE TABLE ${usagestats_db}.sarc_downloads_stats_tmp
    USING PARQUET
AS
SELECT
    s.source,
...
ON d.oid LIKE CONCAT('%', s.repository, '%')
    ...
    JOIN ${stats_db}.result_pids ro
    ON s.rid = ro.pid AND ro.type = 'Digital Object Identifier'
WHERE s.metric_type = 'ft_total'
  AND s.source = 'SARC-OJS'; /*EOS*/