...
function copydb() {
  ...
  for i in ${entities_on_ocean[@]}; do # Use un-quoted values, as the elements are single-words.
    ...
    if [ -n "$create_view_statement_test" ]; then
      ...
      ((num_tables++))
      CURRENT_PRQ_FILE=`hdfs dfs -conf ${IMPALA_CONFIG_FILE} -ls -C "${IMPALA_HDFS_DB_BASE_PATH}/${db}.db/${i}/" | grep -v 'Found' | grep -v '_SUCCESS' | grep -v '_impala_insert_staging' |  head -1`
