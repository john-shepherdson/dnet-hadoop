...
public class PrepareRelatedProjectsJob {
	...
	public static void main(final String[] args) throws Exception {
		final ArgumentApplicationParser parser = new ArgumentApplicationParser(
				IOUtils
						.toString(PrepareRelatedProjectsJob.class
		...
		runWithSparkSession(conf, isSparkSessionManaged, spark -> {
			...
			final Dataset<OaBrokerProject> projects = ClusterUtils
					...
					.map((MapFunction<Project, OaBrokerProject>) ConversionUtils::oafProjectToBrokerProject, Encoders.bean(OaBrokerProject.class));

			final Dataset<Relation> rels = ClusterUtils.loadRawRelations(rawRelsPath, ModelConstants.RESULT_PROJECT, spark);

			final Dataset<RelatedProject> dataset = rels
					.joinWith(projects, projects.col("openaireId").equalTo(rels.col("target")), "inner")
					.map((MapFunction<Tuple2<Relation, OaBrokerProject>, RelatedProject>) t -> new RelatedProject(
							t._1.getSource(), t._2), Encoders.bean(RelatedProject.class));
